/* Compilar con VBCC:
vc +tos16 -o sysinfo.prg sysinfo.c -lgem16

Por alguna razón, si se usa -O2, no funciona bien. No sale
ni un solo printf() en main.
*/
...
static const char *video_name(long vdo) {
  long vdo_id = vdo & 0xffff;
  if (vdo_id == 0)
    vdo_id = (vdo >> 16) & 0xffff;
...
int main(void) {
  ...
  printf("System info para Atari ST (un ejercicio)\n");
  printf("----------------------------------------\n\n");
  printf("Memoria libre: %ld KB\n", mem_free/1024);
  printf("       GEMDOS: %u.%02X\n", tosver & 0xFF, (tosver>>8)&0xFF);
  ...
  if (get_cookie(COOKIE('_', 'A', 'K', 'P'), &value)) {
    printf("       Idioma: 0x%08lx\n", value);
  } else {
    printf("       Idioma: no disponible (cookie _AKP)\n");
  }
